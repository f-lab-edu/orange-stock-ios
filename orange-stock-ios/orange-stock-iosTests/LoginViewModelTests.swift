...
final class LoginViewModelTests: XCTestCase {
    ...
    func testSuccessSaveKeychain() {
        authorizationController.completedWith(credential: Credential(user: "apple ID"))
        XCTAssertTrue(loginVM.isSucceedLogin.value, "애플 로그인 성공 후 키체인 저장 성공!")
    }
    /// 로그인 성공, 키체인 저장 실패
    func testFailSaveKeychain() {
        authorizationController.completedWith(credential: Credential(user: ""))
    ...
    func testFailSaveKeychainMessage() {
        authorizationController.completedWith(credential: Credential(user: ""))
        XCTAssertEqual(loginVM.errorMessage.value, LoginError.invalidCredentials.localizedDescription, "애플로그인 유효하지 않은 자격증명")
    }
    /// 로그인 실패 - Fail Login
